...
test('进error-stats刷新后截图', async ({page,request})=>{
  const u='rf_'+Date.now();
  ...
  await page.screenshot({path:'rf-before.png',fullPage:true});
  await page.reload();
  await page.waitForTimeout(2000);
  console.log('[RF] 刷新后URL:', page.url(), 'canvas:', await page.locator('canvas').count());
  await page.screenshot({path:'rf-after.png',fullPage:true});
});
